...
function PPU:run()
    ...
    if not self.fiber then
        ...
        self.fiber = self.bound_main_loop
        self.vCoroutineState = {}
        --prof.pop("fiber_if")
        --return
    end
    --prof.pop("fiber_if")
...
function PPU:orig_main_loop()
    ...
    if self.vCoroutineState.postExec then
        self.vCoroutineState.postExec()
        self.vCoroutineState.postExec = nil
    end

    -- wait for boot
    if not self.vCoroutineState.boot then
        self:boot()
        self.vCoroutineState.boot = true
        self.vCoroutineState.state = 1
        return self:wait_frame()
    else
        while true do
            local ret
            if self.vCoroutineState.state == 1 then --pre_render
                self.vblank = false
                self.sp_overflow = false
                self.sp_zero_hit = false
                self.vblanking = false
                ...
                self.vCoroutineState.counter = 1
                self.vCoroutineState.counter2 = 1
                self.vCoroutineState.state = 2
            elseif self.vCoroutineState.state == 2 then
                if self.vCoroutineState.counter2 == 1 then
                    self:open_name()
                    ret = self:wait_two_clocks()
                elseif self.vCoroutineState.counter2 == 2 then
                    self:open_attr()
                    ret = self:wait_two_clocks()
                elseif self.vCoroutineState.counter2 == 3 then
                    self:open_pattern(self.bg_pattern_base)
                    ret = self:wait_two_clocks()
                elseif self.vCoroutineState.counter2 == 4 then
                    self:open_pattern(bor(self.io_addr, 8))
                    ret = self:wait_two_clocks()
                end
                self.vCoroutineState.counter2 = self.vCoroutineState.counter2 + 1
                if self.vCoroutineState.counter2 > 4 then
                    self.vCoroutineState.counter2 = 1
                    if self.vCoroutineState.counter >= 32 then
                        self.vCoroutineState.counter = 1
                        self.vCoroutineState.state = 3
                    else
                        self.vCoroutineState.counter = self.vCoroutineState.counter + 1
                    end
                end
            elseif self.vCoroutineState.state == 3 then
                if self.vCoroutineState.counter2 == 1 then
                    --597.step(653, 8) do
                    -- when 597, 605, ..., 653
                    if self.any_show and self.hclk == 645 then
                ...
                elseif self.vCoroutineState.counter2 == 5 then
                    ret = self:wait_zero_clocks()
                end
                self.vCoroutineState.counter2 = self.vCoroutineState.counter2 + 1
                if self.vCoroutineState.counter2 > 5 then
                    self.vCoroutineState.counter2 = 1
                    if self.vCoroutineState.counter >= 8 then
                        if ret == nil then
                            self.on_scanline_0()
                        else
                            self.vCoroutineState.postExec = self.on_scanline_0
                        end
                        self.vCoroutineState.counter = 1
                        self.vCoroutineState.state = 4
                    else
                        self.vCoroutineState.counter = self.vCoroutineState.counter + 1
                    end
                end
            elseif self.vCoroutineState.state == 4 then --pre_render_scanline
                if self.vCoroutineState.counter == 1 then
                    -- when 320
                    self:load_extended_sprites()
                    self:open_name()
                    if self.any_show then
                    ...
                    self.sp_index = 0
                    self.evaluate_sprites_odd_phase = self.evaluate_sprites_odd_phase_0
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 2 then
                    -- when 321
                    self:fetch_name()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 3 then
                    -- when 322
                    self:open_attr()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 4 then
                    -- when 323
                    self:fetch_attr()
                    self:scroll_clock_x()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 5 then
                    -- when 324
                    self:open_pattern(self.io_pattern)
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 6 then
                    -- when 325
                    self:fetch_bg_pattern_0()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 7 then
                    -- when 326
                    self:open_pattern(bor(self.io_pattern, 8))
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 8 then
                    -- when 327
                    self:fetch_bg_pattern_1()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 9 then
                    -- when 328
                    self:preload_tiles()
                    self:open_name()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 10 then
                    -- when 329
                    self:fetch_name()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 11 then
                    -- when 330
                    self:open_attr()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 12 then
                    -- when 331
                    self:fetch_attr()
                    self:scroll_clock_x()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 13 then
                    -- when 332
                    self:open_pattern(self.io_pattern)
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 14 then
                    -- when 333
                    self:fetch_bg_pattern_0()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 15 then
                    -- when 334
                    self:open_pattern(bor(self.io_pattern, 8))
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 16 then
                    -- when 335
                    self:fetch_bg_pattern_1()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 17 then
                    -- when 336
                    if self.any_show and self.scanline + 1 == self.scanline_counter_target and self.scanline_counter_target ~= 0 then
                        self.inform_scanline_counter = true
                        self.inform_scanline_counter_clk = true
                    ...
                    self:open_name()
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 18 then
                    -- when 337
                    if self.any_show then
                        self:update_enabled_flags_edge()
                        if self.scanline == PPU.SCANLINE_HDUMMY and self.odd_frame then
                    ...
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter == 19 then
                    -- when 338
                    self:open_name()
                    self.scanline = self.scanline + 1
                end
                ...
                self.vCoroutineState.counter = self.vCoroutineState.counter + 1
                if self.vCoroutineState.counter > 19 then
                    self.vCoroutineState.state = 5
                end
            elseif self.vCoroutineState.state == 5 then
                if self.scanline ~= SCANLINE_VBLANK then
                    ...
                    self.vCoroutineState.counter = 1
                    self.vCoroutineState.counter2 = 1
                    self.vCoroutineState.state = 6
                else
                    self.on_scanline_240()
                    self.hclk = PPU.HCLOCK_VBLANK_0
                    self.vCoroutineState.counter = 1
                    self.vCoroutineState.state = 8
                end
                ret = self:wait_zero_clocks()
            elseif self.vCoroutineState.state == 6 then --render_scanline (need write)
                if self.vCoroutineState.counter2 == 1 then
                    --0.step(248, 8) do
                    -- when 0, 8, ..., 248
                    if self.any_show then
                ...
                elseif self.vCoroutineState.counter2 == 5 then
                    ...
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter2 == 6 then
                    if self.any_show then
                        self:fetch_bg_pattern_0()
                        if self.hclk >= 64 then
                            self:evaluate_sprites_odd()
                    ...
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter2 == 7 then
                    if self.any_show then
                        if self.hclk >= 64 then
                            self:evaluate_sprites_even()
                        end
                    ...
                    ret = self:wait_one_clock()
                elseif self.vCoroutineState.counter2 == 8 then
                    if self.any_show then
                        self:fetch_bg_pattern_1()
                        if self.hclk >= 64 then
                            self:evaluate_sprites_odd()
                ...
                self.vCoroutineState.counter2 = self.vCoroutineState.counter2 + 1
                if self.vCoroutineState.counter2 > 8 then
                    self.vCoroutineState.counter2 = 1
                    if self.vCoroutineState.counter >= 32 then
                        self.vCoroutineState.counter = 1
                        self.vCoroutineState.counter2 = 1
                        self.vCoroutineState.state = 7
                    else
                        self.vCoroutineState.counter = self.vCoroutineState.counter + 1
                    end
                end
            elseif self.vCoroutineState.state == 7 then --post_render_scanline (need write)
                if self.vCoroutineState.counter2 == 1 then
                    --256.step(312, 8) do
                    if self.hclk == 256 then
                        -- when 256
                ...
                self.vCoroutineState.counter2 = self.vCoroutineState.counter2 + 1
                if self.vCoroutineState.counter2 > 6 then
                    self.vCoroutineState.counter2 = 1
                    if self.vCoroutineState.counter >= 8 then
                        self.vCoroutineState.state = 4
                        self.vCoroutineState.counter = 1
            ...
            elseif self.vCoroutineState.state == 8 then --post_render
                if self.vCoroutineState.counter == 1 then
                    -- when 681
                    self:vblank_0()
                    ret = self:wait_zero_clocks()
                ...
                self.vCoroutineState.counter = self.vCoroutineState.counter + 1
                if self.vCoroutineState.counter > 3 then
                    self.vCoroutineState.state = 1
                end
            end
            if ret ~= nil then
                --print("ret", self.vCoroutineState)
                return ret
            else
                --print("math", self.vCoroutineState)
            end
        end
